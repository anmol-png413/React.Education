...
const Courses2 = () => {
  // const { slug } = useParams();
  const { slug, courseSlug } = useParams(); // ✅ courseSlug bhi add karo
...
const handleBrochureClick = (course) => {
  ...
  // ✅✅✅ YE NAYA CODE HAI - SCROLL FIX KAREGA ✅✅✅
  setTimeout(() => {
    const coursesHeading = document.querySelector('h2');
    if (coursesHeading && coursesHeading.textContent.includes('Programmes offered by')) {
      coursesHeading.scrollIntoView({ 
        behavior: 'smooth', 
        block: 'center' 
      });
    }
  ...
  if (courseSlug && courseData.length > 0) {
...
}, [slug, page, currentFilters, courseSlug, courseData]);
